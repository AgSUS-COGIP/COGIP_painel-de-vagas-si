...
const DASH_SQL = {
    ...
    custo_dim AS (
      SELECT
        ...
        (COALESCE(c.SALARIO_BASE,0) + COALESCE(c.INSALUBRIDADE_PERICULOSIDADE,0) + COALESCE(c.GRATIFICACAO_RT,0)
          + COALESCE(c.NOTURNO,0) + COALESCE(c.ENCARGOS,0) + COALESCE(c.PROVISOES,0) + COALESCE(c.VALE_ALIMENTACAO,0)) AS mensal_unitario
  ...
  REMANEJAMENTO_CADASTRO: `
    ...
    SELECT
      ...
      COALESCE(cgv.VALE_ALIMENTACAO, 0) AS vale_alimentacao,
      (
        ...
        COALESCE(cgv.VALE_ALIMENTACAO, 0)
      ) AS valor_mensal,
...
module.exports = { DASH_SQL, montarCaseCargoSql, montarSqlRemanejamentoLista };
