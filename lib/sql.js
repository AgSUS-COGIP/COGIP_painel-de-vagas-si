...
// Componentes de custo mensal de uma vaga, na ORDEM em que aparecem na tabela
// CUSTO_GERAL_VAGA e no "Resumo dos Valores (Mensal)" do remanejamento. Fonte única da
// verdade: o ALTER TABLE (garantirColunasCustoGeralVaga), as somas do mensal e os
// campos devolvidos ao front saem todos daqui — incluir um componente novo é
// acrescentar uma linha nesta lista (+ a linha na tabela do HTML).
//   coluna       -> nome no banco
//   aliasCadastro-> alias na query de cadastro (REMANEJAMENTO_CADASTRO)
//   aliasDetalhe -> alias na query de detalhe da movimentação
//   campo        -> nome do campo no JSON do front
const COMPONENTES_CUSTO_VAGA = [
  { coluna: "SALARIO_BASE", aliasCadastro: "salario_base", aliasDetalhe: "salario", campo: "salarioBase" },
  { coluna: "INSALUBRIDADE_PERICULOSIDADE", aliasCadastro: "insalubridade_periculosidade", aliasDetalhe: "insal", campo: "insalubridadePericulosidade" },
  { coluna: "GRATIFICACAO_RT", aliasCadastro: "gratificacao_rt", aliasDetalhe: "grat", campo: "gratificacaoRt" },
  { coluna: "NOTURNO", aliasCadastro: "adicional_noturno", aliasDetalhe: "noturno", campo: "adicionalNoturno" },
  { coluna: "ENCARGOS", aliasCadastro: "encargos", aliasDetalhe: "encargos", campo: "encargos" },
  { coluna: "PROVISOES", aliasCadastro: "provisoes", aliasDetalhe: "provisoes", campo: "provisoes" },
  { coluna: "VALE_ALIMENTACAO", aliasCadastro: "vale_alimentacao", aliasDetalhe: "vale", campo: "valeAlimentacao" },
  { coluna: "ABONO_EMERGENCIAL", aliasCadastro: "abono_emergencial", aliasDetalhe: "abono", campo: "abonoEmergencial" },
  { coluna: "TRABALHO_EM_CAMPO", aliasCadastro: "trabalho_em_campo", aliasDetalhe: "campo", campo: "trabalhoEmCampo" },
  { coluna: "CAPTACAO_MEDICA", aliasCadastro: "captacao_medica", aliasDetalhe: "captacao", campo: "captacaoMedica" },
  { coluna: "AUXILIO_AREA_REMOTA", aliasCadastro: "auxilio_area_remota", aliasDetalhe: "remota", campo: "auxilioAreaRemota" }
];

// Colunas acrescentadas à tabela depois da criação (as 4 últimas da lista acima) —
// as que garantirColunasCustoGeralVaga cria por ALTER TABLE quando faltam.
const COLUNAS_CUSTO_NOVAS = ["ABONO_EMERGENCIAL", "TRABALHO_EM_CAMPO", "CAPTACAO_MEDICA", "AUXILIO_AREA_REMOTA"];

// Soma SQL do valor mensal unitário da vaga, a partir de um alias da tabela de custo.
function somaMensalCustoSql(alias) {
  return COMPONENTES_CUSTO_VAGA.map(c => `COALESCE(${alias}.${c.coluna},0)`).join(" + ");
}

// Coluna opcional de admissões programadas (ver MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA
// em config): só entra na consulta quando a flag está ligada e a coluna existe na
// view. Fora disso, o mapeamento assume 0 e nada muda nos painéis.
const COL_ADM_PROG_DETALHE = DASH_CONFIG.MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA
  ? "      `admissao_programada`,\n"
  : "";
const COL_ADM_PROG_TOTAIS = DASH_CONFIG.MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA
  ? ",\n      SUM(`admissao_programada`) AS `admissao_programada`"
  : "";
...
${COL_ADM_PROG_DETALHE}
  ...
  MONITORAMENTO_TOTAIS: `
    SELECT
      SUM(\`total_contratados_geral\`) AS \`total_contratados_geral\`${COL_ADM_PROG_TOTAIS}
    FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.MONITORAMENTO_VIEW}\`
  `,
  REMANEJAMENTO_LISTA: `
    WITH
    ...
    SELECT
      ...
      -- Período: no remanejamento normal (TP_AJUSTE='N') os dois lados usam N_MESES.
      -- No ajuste pontual ('S'), o REDUZIDO usa N_MESES (= 13 - o número digitado no
      -- campo do mês) e o ACRESCENTADO usa o próprio número digitado (13 - N_MESES) —
      -- mesma regra do formulário e do detalhe.
      (COALESCE(red.total_mensal, 0) * COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO))) AS total_reduzido_periodo,
      (COALESCE(acr.total_mensal, 0) * CASE WHEN COALESCE(p.TP_AJUSTE, 'N') = 'S'
          THEN GREATEST(1, 13 - COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO)))
          ELSE COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO)) END) AS total_acrescentado_periodo,
      ((COALESCE(acr.total_mensal, 0) * CASE WHEN COALESCE(p.TP_AJUSTE, 'N') = 'S'
          THEN GREATEST(1, 13 - COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO)))
          ELSE COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO)) END)
        - (COALESCE(red.total_mensal, 0) * COALESCE(p.N_MESES, 13 - MONTH(p.DATA_INSERCAO)))) AS impacto_periodo,
  ...
  REMANEJAMENTO_CADASTRO: `
    ...
    custo_dim AS (
      SELECT
        c.ID_DSEI_CASAI,
        c.ID_VAGA,
        ${COMPONENTES_CUSTO_VAGA.map(c => `c.${c.coluna}`).join(",\n        ")}
      FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.CUSTO_GERAL_VAGA_TABLE}\` c
      JOIN (
        SELECT ID_DSEI_CASAI, ID_VAGA, MAX(ID_CUSTO_GERAL_VAGA) AS max_id
        FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.CUSTO_GERAL_VAGA_TABLE}\`
    ...
    SELECT
      ...
      ${COMPONENTES_CUSTO_VAGA.map(c => `COALESCE(cgv.${c.coluna}, 0) AS ${c.aliasCadastro}`).join(",\n      ")},
      (${somaMensalCustoSql("cgv")}) AS valor_mensal,
...
// Lista de remanejamentos filtrada por TP_AJUSTE: 'N' = histórico (remanejamentos
// normais), 'S' = ajustes pontuais. Reaproveita a query REMANEJAMENTO_LISTA,
// injetando o WHERE antes do ORDER BY final. `tp` é literal controlado (não vem do
// usuário), então a interpolação é segura. Registros antigos sem TP_AJUSTE contam
// como 'N' (COALESCE), aparecendo no histórico.
function montarSqlRemanejamentoLista(tp) {
  const filtro = tp === "S" ? "S" : "N";
  return DASH_SQL.REMANEJAMENTO_LISTA.replace(
    /ORDER BY p\.DATA_INSERCAO DESC\s*$/,
    `WHERE COALESCE(p.TP_AJUSTE, 'N') = '${filtro}'\n    ORDER BY p.DATA_INSERCAO DESC`
  );
}
...
module.exports = {
  DASH_SQL,
  montarCaseCargoSql,
  montarSqlRemanejamentoLista,
  COMPONENTES_CUSTO_VAGA,
  COLUNAS_CUSTO_NOVAS,
  somaMensalCustoSql
};
