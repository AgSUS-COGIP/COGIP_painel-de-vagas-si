...
const { obterEscopoUsuarioComConn } = require("./escopo");
...
async function garantirTabelaUsuarios() {
  ...
  try {
    ...
    if (total === 0) {
      ...
      if (senhaEnv) {
      ...
      } else if (/^(1|true|sim|yes)$/i.test(String(process.env.SEED_ADMIN_LOG_SENHA || "").trim())) {
        // Opt-in EXPLÍCITO (ex.: desenvolvimento local): só aqui a senha aleatória
        // aparece em log. Em produção, deixe SEED_ADMIN_LOG_SENHA desligada.
        console.warn(
          `[ATENÇÃO] Usuário administrador inicial criado (login: "${login}").\n` +
        ...
        // Padrão seguro: a senha gerada NUNCA é escrita em log (logs podem ser
        // agregados/persistidos e vazariam a credencial inicial).
        console.warn(
          `[ATENÇÃO] Usuário administrador inicial criado (login: "${login}") com senha aleatória ` +
          "NÃO registrada em log. Defina SEED_ADMIN_SENHA no .env (ou SEED_ADMIN_LOG_SENHA=1 apenas " +
          "em ambiente local) e recrie o admin para obter uma credencial conhecida."
        );
      }
    } else {
  ...
  getMysqlConnection()
    .then(async (conn) => {
      try {
        ...
        // Escopo de dados por DSEI (multi-tenancy por linha). Anexado aqui para os
        // handlers/queries filtrarem por DSEI. Best-effort: em caso de falha, mantém
        // o padrão "vê tudo" (alinhado ao default ACESSO_TODOS_DSEIS=1).
        try {
