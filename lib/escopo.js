...
// Resolve o id da unidade orçamentária (DSEI/CASAI) a partir do NOME, casando por
// chave normalizada contra a MESMA fonte dos ids do escopo (VW_SAUDE_INDIGENA).
// Retorna null se não casar. Os nomes de DSEI/CASAI usados no cadastro de acesso
// vêm da mesma base (mesma grafia), então o casamento é exato na prática.
async function resolverIdUnidadePorNomeComConn(conn, nome) {
  const chave = normalizarChaveDash(nome);
  if (!chave) return null;
  const unidades = await listarDseisComConn(conn); // [{id, nome}]
  const achou = (unidades || []).find(u => normalizarChaveDash(u.nome) === chave);
...
module.exports = {
  ...
  resolverIdUnidadePorNomeComConn,
  escopoSqlDsei,
  dseiNoEscopo,
  erroEscopo,
