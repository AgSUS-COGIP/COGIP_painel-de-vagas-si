...
const STATUS_LABEL_TO_DB = {
  ...
  "Aguardando devolutiva do DSEI": "AGUARDANDO_COMPLEMENTACAO",
  "Concluída": "CONCLUIDO",
...
function mapearPedido(row) {
  ...
  return {
    ...
    observacoesSancao: limparValorDash(row.obs_sancao) || "—",
    anexos: []
  };
}
...
const SELECT_PEDIDOS = `
  SELECT
    p.id_pedido, p.matricula, p.num_processo_sei,
    ...
    s.aplicado_por, s.nome_documento, s.obs AS obs_sancao,
...
async function buscarTrabalhadoresComConn(conn, termo) {
  ...
  const [rows] = await conn.query(
    `SELECT \`MATRICULA\` AS matricula, \`NOME\` AS nome,
            ...
            \`LOCAL_TRABALHO_DESC\` AS polo
       FROM ${T_TRABALHADOR}
      WHERE \`NOME\` LIKE ? OR \`MATRICULA\` LIKE ?
      ORDER BY \`NOME\` LIMIT 25`,
    [like, like]
  ).catch(async () => {
    // Fallback caso a coluna de polo (LOCAL_TRABALHO_DESC) não exista na tabela.
    return conn.query(
      `SELECT \`MATRICULA\` AS matricula, \`NOME\` AS nome,
              \`CARGO_ATUAL_DESC\` AS cargo, \`UNIDADE_ORCAMENTARIA_DESC\` AS dsei,
              NULL AS polo
  ...
  try {
    const [resPedido] = await conn.execute(
      `INSERT INTO ${T_PEDIDO}
         (matricula, nome_trabalhador_snapshot, cargo_snapshot, dsei_casai_snapshot, polo_base_snapshot,
    ...
    const [resDemanda] = await conn.execute(
      `INSERT INTO ${T_DEMANDA}
         (id_pedido, responsavel_atual, atendimento, status, ultima_atualizacao_status)
       VALUES (?, ?, 'PENDENTE', ?, NOW())`,
...
async function atualizarSancaoComConn(conn, idPedido, patch, usuario) {
  ...
  const campos = {
    ...
    obs: p.observacoesSancao !== undefined ? texto(p.observacoesSancao) : undefined,
  ...
  if (existente) {
    ...
    await conn.execute(
      `INSERT INTO ${T_SANCAO}
         (id_pedido, id_categoria, dias_suspensao, motivo, data_sancao, aplicado_por, nome_documento, obs)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        id, idCategoria,
        ...
        campos.obs === undefined ? null : campos.obs
      ]
    );
  }
  return obterPedidoComConn(conn, id);
