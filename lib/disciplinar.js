...
const { dseiNoEscopo, erroEscopo, escopoSqlDsei } = require("./escopo");
...
// Lança 403 se o pedido for de um DSEI fora do escopo do usuário.
async function garantirEscopoPedidoComConn(conn, idPedido, escopo) {
  if (!escopo || escopo.todos) return;
  const id = Number(idPedido);
  const [rows] = await conn.query(`SELECT \`id_dsei_casai\` AS id FROM ${T_PEDIDO} WHERE id_pedido = ? LIMIT 1`, [id]);
  const idDsei = rows && rows[0] ? rows[0].id : null;
  if (!dseiNoEscopo(escopo, idDsei)) throw erroEscopo("Pedido fora do seu escopo de DSEI.");
}

// Lança 403 se o anexo pertencer a um pedido de DSEI fora do escopo.
async function garantirEscopoAnexoComConn(conn, idAnexo, escopo) {
  if (!escopo || escopo.todos) return;
  const id = Number(idAnexo);
  const [rows] = await conn.query(
    `SELECT p.\`id_dsei_casai\` AS id FROM ${T_ANEXO} a JOIN ${T_PEDIDO} p ON p.id_pedido = a.id_pedido WHERE a.id_anexo = ? LIMIT 1`,
    [id]
  );
  const idDsei = rows && rows[0] ? rows[0].id : null;
  if (!dseiNoEscopo(escopo, idDsei)) throw erroEscopo("Anexo fora do seu escopo de DSEI.");
}
...
async function buscarTrabalhadoresComConn(conn, termo, escopo) {
  ...
  // Escopo por DSEI: usuário restrito só encontra trabalhadores do(s) seu(s) DSEI(s).
  const esc = escopoSqlDsei(escopo, "`UNIDADE_ORCAMENTARIA_ID`");
  const [rows] = await conn.query(
    `SELECT \`MATRICULA\` AS matricula, \`NOME\` AS nome,
       ...
       FROM ${T_TRABALHADOR}
      WHERE (\`NOME\` LIKE ? OR \`MATRICULA\` LIKE ?)${esc.sql}
      ORDER BY \`NOME\` LIMIT 25`,
    [like, like, ...esc.params]
  );
  return (rows || []).map(r => ({
...
// ---------- Criação de pedido ----------
async function criarPedidoComConn(conn, body, criadoPor, oficioFile, anexos, anexosTipos, escopo) {
  const dados = body || {};
  ...
  // Escopo: usuário restrito não abre pedido para trabalhador de outro DSEI.
  if (!dseiNoEscopo(escopo, idDseiCasai)) throw erroEscopo("Você não pode abrir pedido para trabalhador de DSEI fora do seu escopo.");
...
module.exports = {
  ...
  garantirEscopoPedidoComConn,
  garantirEscopoAnexoComConn,
