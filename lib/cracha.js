...
function montarSelectCrachas(porMatricula) {
  ...
  // A base de crachás não tem o id do DSEI, só o nome. Para o escopo por DSEI,
  // trazemos o UNIDADE_ORCAMENTARIA_ID do consolidado pela MATRICULA. Importados
  // (perna do overlay) não têm vínculo na base => ID_DSEI_CASAI nulo (filtrados
  // por nome, ver getCrachaData).
  return `
    SELECT
      ...
      COALESCE(m.\`LINK_FOTOS\`, b.\`LINK_FOTOS\`)      AS LINK_FOTOS,
      b.\`DATA_ADMISSAO\`                             AS DATA_ADMISSAO,
    ...
    LEFT JOIN \`${SCHEMA}\`.\`${DASH_CONFIG.TRABALHADOR_CONSOLIDADO_TABLE}\` tc ON tc.\`MATRICULA\` = b.\`MATRICULA\`
    ${filtroBase}
    UNION ALL
    SELECT
