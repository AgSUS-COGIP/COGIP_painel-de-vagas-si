...
function mapearLinha(row) {
  ...
  const situacaoDetalhada = limparValorDash(row.SITUACAO_DETALHADA);
  return {
    ...
    situacaoDetalhada,
    // Vínculo ativo = qualquer situação que não seja desligamento (inclui
    // férias, atestado, licenças e afastamentos).
    ativo: !/desligamento/i.test(situacaoDetalhada),
    status,
