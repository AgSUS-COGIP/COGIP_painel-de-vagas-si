...
// TABELA ÚNICA: com o ETL desativado, identidade e controle vivem juntos em
// UGP_CRACHAS_CONTROLE_MANUAL (uma linha por trabalhador). A antiga base do ETL
// (UGP_CONTROLE_CRACHAS_SI) foi migrada para cá uma única vez (ver
// scripts/unificar-cracha.js) e agora serve só de backup — o app não a lê mais.
// CPF e o id do DSEI (escopo) continuam vindo do consolidado por join (MATRICULA).
...
// MANUAL é a tabela única do módulo. BASE é a antiga base do ETL, mantida apenas
// como backup pós-unificação (não é mais lida pelo app; usada só pela migração).
const BASE = `\`${SCHEMA}\`.\`${DASH_CONFIG.CRACHAS_TABLE}\``; // eslint-disable-line no-unused-vars
...
const COLUNAS_EXTRAS = [
  ...
  // Identidade do trabalhador (após a unificação, vale para TODOS — antes só
  // era gravada para importados; a migração preencheu os demais a partir da base).
  ...
  // Link da foto (override manual, sobrevive à recarga do ETL da base).
  { nome: "LINK_FOTOS", ddl: "`LINK_FOTOS` VARCHAR(1000) NULL" },
  // Identidade/dados que antes vinham só da base do ETL. Com a unificação (sem
  // mais ETL), passam a viver aqui para todos os trabalhadores. Preenchidos pela
  // migração (scripts/unificar-cracha.js) a partir da base.
  { nome: "MOTIVO_NAO_CRACHA", ddl: "`MOTIVO_NAO_CRACHA` VARCHAR(255) NULL" },
  { nome: "DATA_ADMISSAO", ddl: "`DATA_ADMISSAO` DATE NULL" },
...
// Leitura da tabela única (uma linha por MATRICULA), com join no consolidado
// só para CPF e id do DSEI. montarSelectCrachas() monta esse SELECT.
// Nome da coluna de CPF na base consolidada (varia entre bases: CPF, NR_CPF,
// NUM_CPF...). Detectado uma vez via information_schema. undefined = ainda não
// detectado; null = não existe (CPF sai vazio).
let _colCpf = undefined;
async function detectarColunaCpfComConn(conn) {
  if (_colCpf !== undefined) return _colCpf;
  try {
    const [cols] = await conn.query(
      `SELECT COLUMN_NAME FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ? AND UPPER(COLUMN_NAME) LIKE '%CPF%'`,
      [SCHEMA, DASH_CONFIG.TRABALHADOR_CONSOLIDADO_TABLE]
    );
    const nomes = (cols || []).map(c => String(c.COLUMN_NAME));
    _colCpf = nomes.find(n => n.toUpperCase() === "CPF") || nomes[0] || null;
  } catch (e) { _colCpf = null; }
  return _colCpf;
}
const selectCpf = () => (_colCpf ? `tc.\`${_colCpf}\`` : "NULL");

function montarSelectCrachas(porMatricula) {
  const filtro = porMatricula ? "WHERE m.`MATRICULA` = ?" : "";
  // Tabela única (${MANUAL}): identidade + controle no mesmo registro. O CPF e o
  // id do DSEI (para o escopo por DSEI) continuam vindo do consolidado por join,
  // pela MATRICULA (essa base ainda é atualizada). Sem vínculo no consolidado =>
  // ID_DSEI_CASAI nulo (o escopo cai no filtro por nome; ver getCrachaData).
  return `
    SELECT
      ...
      m.\`CARGO\`                                     AS CARGO,
      m.\`DSEI\`                                      AS DSEI,
      tc.\`UNIDADE_ORCAMENTARIA_ID\`                  AS ID_DSEI_CASAI,
      m.\`SITUACAO_DETALHADA\`                        AS SITUACAO_DETALHADA,
      m.\`LINK_FOTOS\`                                AS LINK_FOTOS,
      DATE_FORMAT(m.\`DATA_ADMISSAO\`, '%d/%m/%Y')     AS DATA_ADMISSAO,
      m.\`MOTIVO_NAO_CRACHA\`                         AS MOTIVO,
      m.\`STATUS_MANUAL\`                             AS STATUS_EFETIVO,
      m.\`STATUS_MANUAL\`                             AS STATUS_MANUAL,
      ${COLS_OVERLAY_SELECT}
    FROM ${MANUAL} m
    LEFT JOIN \`${SCHEMA}\`.\`${DASH_CONFIG.TRABALHADOR_CONSOLIDADO_TABLE}\` tc ON tc.\`MATRICULA\` = m.\`MATRICULA\`
    ${filtro}
  `;
}
...
function mapearLinha(row) {
  ...
  return {
    ...
    // Tabela unificada: não há mais distinção base/overlay (ETL desativado).
    importado: false,
    // Há "desfazer" quando existe um snapshot a restaurar (PREV_TINHA = 1).
    // PREV_TINHA = 0 é criação de trabalhador (import) — desfazer não se aplica.
...
// Resolve o DSEI (UNIDADE_ORCAMENTARIA_ID) de uma matrícula via consolidado.
// Importados sem vínculo no consolidado retornam null (fail-closed p/ o restrito).
async function dseiDaMatriculaComConn(conn, matricula) {
  const mat = limparValorDash(matricula);
  if (!mat) return null;
  try {
    const [rows] = await conn.query(
      `SELECT \`UNIDADE_ORCAMENTARIA_ID\` AS id FROM \`${SCHEMA}\`.\`${DASH_CONFIG.TRABALHADOR_CONSOLIDADO_TABLE}\`
...
// Lança 403 se a matrícula não estiver no escopo do usuário.
...
async function getCrachaData(forcar = false, escopo = null, incluirCpf = false) {
  ...
  // Escopo de DSEI: filtra por id (vindo do consolidado via matrícula). Importados
  // não têm id na base => caem no filtro por nome normalizado (fail-closed).
  let rows = todas;
...
const COLS_SNAPSHOT = [
  ...
  "MOTIVO_SEGUNDA_VIA", "NOME", "CARGO", "DSEI", "SITUACAO_DETALHADA", "OBSERVACAO_CRACHA", "LINK_FOTOS"
];
...
async function capturarSnapshotAnterior(conn, mat) {
  const [rows] = await conn.query(
    `SELECT
       ...
       \`NOME\`, \`CARGO\`, \`DSEI\`, \`SITUACAO_DETALHADA\`, \`OBSERVACAO_CRACHA\`, \`LINK_FOTOS\`
...
// Edição manual pela tela: exige que a matrícula já exista na tabela. Não cria
// trabalhador novo aqui. A identidade (nome/cargo/DSEI) não é editável por aqui.
async function salvarControleComConn(conn, matricula, campos, usuario) {
  const mat = limparValorDash(matricula);
  if (!mat) throw new Error("Matrícula não informada.");

  const [existe] = await conn.query(`SELECT 1 FROM ${MANUAL} WHERE \`MATRICULA\` = ? LIMIT 1`, [mat]);
  if (!existe.length) throw new Error("Trabalhador não encontrado na base de crachás.");

  // Identidade só pode ser definida via importação (de quem ainda não existe).
...
// Importação de planilha. Para cada linha: se a matrícula já existe, atualiza só
// os campos de controle informados; se não existe, cria o trabalhador com a
// identidade da planilha (Nome obrigatório).
...
// guardado no snapshot (undo de 1 nível). Não desfaz a criação de um cadastro.
async function reverterControleComConn(conn, matricula) {
  ...
  if (Number(prevTinha) === 0) {
    // A alteração anterior foi a CRIAÇÃO do trabalhador (não havia registro antes).
    // Desfazer apagaria o cadastro inteiro — não permitido por aqui.
...
// Cada matrícula é tratada individualmente; as que não têm o que desfazer (ou
// falham) entram em `erros`, sem interromper as demais.
async function reverterLoteComConn(conn, matriculas) {
  ...
  for (const mat of lista) {
    try {
      registros.push(await reverterControleComConn(conn, mat));
    } catch (err) {
      erros.push({ matricula: mat, erro: err && err.message ? err.message : "Falha." });
    }
  }
...
// Garante que a matrícula já existe na tabela — não cria trabalhador novo só
// por causa da foto.
