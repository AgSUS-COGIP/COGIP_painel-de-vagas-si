...
// ---------------------------------------------------------------------------
// Ajustes pontuais (admin) — movimentações SEM ID_PROCESSO_REMANEJAMENTO.
//
// Diferente do remanejamento por processo, o ajuste pontual grava direto em
// MOVIMENTACAO_REMANEJAMENTO com ID_PROCESSO_REMANEJAMENTO = NULL, sem nenhuma
// validação de ociosas/impacto/PSS e sem exigir os dois lados (só acréscimo ou
// só decréscimo é permitido). Como não têm processo, não aparecem no histórico
// (que parte de PROCESSO_REMANEJAMENTO) e são vistos/excluídos numa janela à parte.
// ---------------------------------------------------------------------------

// SAMU Indígena tem id/rotulo especiais (espelha lib/sql.js e lib/escopo.js).
const ID_SAMU_REMANEJAMENTO = 9610501;

async function salvarAjusteRemanejamentoComConn(conn, body, escopo) {
  const idDseiCasai = converterNumeroDash(body.idDseiCasai);
  const criadoPor = limparValorDash(body.criadoPor || body.usuario || "painel");
  ...
  if (!idDseiCasai) throw new Error("Selecione o DSEI/CASAI.");
  // Regra única: ao menos uma linha no total. Sem exigir os dois lados, sem
  // validar vagas ociosas, impacto financeiro ou bloqueio por processo seletivo.
  ...
  await conn.beginTransaction();
  try {
    const inserirAjuste = (linha, tipo) => conn.execute(
      `INSERT INTO \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.MOVIMENTACAO_REMANEJAMENTO_TABLE}\` (
        ID_PROCESSO_REMANEJAMENTO, ID_DSEI_CASAI, ID_VAGA, TIPO_MOVIMENTACAO, QTD, CRIADO_POR
      ) VALUES (NULL, ?, ?, ?, ?, ?)`,
      [idDseiCasai, linha.idCargoFuncao, tipo, linha.quantidade, criadoPor || null]
    );

    for (const linha of linhasReduzido) await inserirAjuste(linha, "DECRESCIMO");
    for (const linha of linhasAcrescentado) await inserirAjuste(linha, "ACRESCIMO");

    await conn.commit();
    return { inseridos: linhasReduzido.length + linhasAcrescentado.length };
  } catch (err) {
    await conn.rollback();
    throw err;
  }
...
function montarSqlAjustesRemanejamento() {
  return `
    WITH
    dsei_dim AS (
      SELECT UNIDADE_ORCAMENTARIA_ID AS id_dsei_casai, MAX(UNIDADE_ORCAMENTARIA_DESC) AS dsei_casai
      FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`VW_SAUDE_INDIGENA\`
    ...
    SELECT
      m.ID_MOVIMENTACAO_REMANEJAMENTO AS id_movimentacao,
      m.ID_DSEI_CASAI AS id_dsei_casai,
      CASE WHEN m.ID_DSEI_CASAI = ${ID_SAMU_REMANEJAMENTO} THEN 'SAMU INDÍGENA'
           ELSE COALESCE(dd.dsei_casai, CONCAT('DSEI/CASAI ID ', m.ID_DSEI_CASAI)) END AS dsei_casai,
      ${montarCaseCargoSql("m.ID_VAGA", "cd")} AS cargo,
      m.TIPO_MOVIMENTACAO AS tipo,
      m.QTD AS qtd,
      m.CRIADO_POR AS criado_por,
      DATE_FORMAT(m.DATA_INSERCAO, '%d/%m/%Y %H:%i') AS data_insercao_formatada
    FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.MOVIMENTACAO_REMANEJAMENTO_TABLE}\` m
    LEFT JOIN cargo_dim cd ON cd.id_cargo_funcao = m.ID_VAGA
    LEFT JOIN dsei_dim dd ON dd.id_dsei_casai = m.ID_DSEI_CASAI
    WHERE m.ID_PROCESSO_REMANEJAMENTO IS NULL
    ORDER BY m.DATA_INSERCAO DESC, m.ID_MOVIMENTACAO_REMANEJAMENTO DESC
  `;
}

function mapAjusteRemanejamentoRow(row) {
  return {
    idMovimentacao: converterNumeroDash(row.id_movimentacao),
    idDseiCasai: converterNumeroDash(row.id_dsei_casai),
    dseiCasai: limparValorDash(row.dsei_casai),
    cargo: limparValorDash(row.cargo),
    tipo: limparValorDash(row.tipo),
    qtd: converterNumeroDash(row.qtd),
    criadoPor: limparValorDash(row.criado_por),
    dataInsercaoFormatada: limparValorDash(row.data_insercao_formatada)
  };
}
...
async function getAjustesRemanejamentoData(escopo) {
  const conn = await getMysqlConnection();
  try {
    const [rows] = await conn.query(montarSqlAjustesRemanejamento());
    const mapeadas = (rows || []).map(mapAjusteRemanejamentoRow);
    return {
      rows: filtrarLinhasPorEscopo(mapeadas, escopo),
      atualizadoEm: ""
    };
  } finally {
    await fecharJdbc(conn);
...
async function excluirAjusteRemanejamentoComConn(conn, idMovimentacao, escopo) {
  const id = converterNumeroDash(idMovimentacao);
  if (!id) throw new Error("Ajuste inválido.");

  const [rows] = await conn.query(
    `SELECT ID_DSEI_CASAI, ID_PROCESSO_REMANEJAMENTO
       FROM \`${DASH_CONFIG.DB_SCHEMA}\`.\`${DASH_CONFIG.MOVIMENTACAO_REMANEJAMENTO_TABLE}\`
      WHERE ID_MOVIMENTACAO_REMANEJAMENTO = ? LIMIT 1`,
    [id]
  );
  const mov = rows && rows[0] ? rows[0] : null;
  if (!mov) throw new Error("Ajuste não encontrado.");

  // Só exclui ajustes pontuais (sem processo). Uma movimentação vinculada a um
  // remanejamento formal nunca é removida por esta via.
  if (mov.ID_PROCESSO_REMANEJAMENTO != null) {
    const err = new Error("Esta movimentação pertence a um remanejamento e não pode ser excluída como ajuste.");
    err.status = 400;
    err.expose = true;
    throw err;
  }

  // Escopo: não excluir ajuste de DSEI fora do escopo do usuário.
  if (!dseiNoEscopo(escopo, mov.ID_DSEI_CASAI)) {
    const err = new Error("Você não tem acesso a este ajuste (DSEI fora do seu escopo).");
    err.status = 403;
    err.expose = true;
    throw err;
  }
