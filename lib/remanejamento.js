...
const { DASH_SQL, montarCaseCargoSql, montarSqlRemanejamentoLista } = require("./sql");
...
// N_MESES: do mês informado (1..12) até o fim do ano (dezembro). Sem mês válido,
// mantém o padrão (do mês atual até dezembro). Vale para os DOIS lados — reduzido e
// acrescentado usam o mesmo período — e para as duas rotas (remanejamento normal, com
// o mês vindo do select, e ajuste pontual, com o mês digitado).
...
// no campo do mês, ou seja, o complemento de N_MESES (13 - N_MESES). No remanejamento
// normal os dois lados usam o mesmo N_MESES.
function mesesLadoAcrescentadoAjuste(meses) {
  return Math.max(1, 13 - Math.max(1, Math.min(12, Math.floor(Number(meses)) || 1)));
}
...
async function getRemanejamentoEdicaoData(idProcesso, escopo) {
  ...
  try {
    ...
    // O formulário repõe um dos dois campos, conforme o tipo do processo:
    //   • remanejamento normal -> o MÊS (meses = 13 - mes, logo mes = 13 - meses);
    //   • ajuste pontual -> o Nº DE MESES, que é o próprio N_MESES.
...
async function getRemanejamentoDetalheData(idProcesso, escopo) {
  ...
  try {
    ...
    // N_MESES e o acrescentado, o número digitado no campo do mês (13 - N_MESES).
