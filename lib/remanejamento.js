...
async function getRemanejamentoEdicaoData(idProcesso, escopo) {
  ...
  try {
    ...
    return {
      ...
      mes,
      meses: (nMeses >= 1 && nMeses <= 12) ? nMeses : null,
...
// Meses (N_MESES) de um ajuste. Prioriza a quantidade digitada (body.meses, 1..12);
// se ausente, usa o mês escolhido (mês → dezembro) e, por fim, o padrão do ano.
function resolverMesesAjuste(body) {
  const mesesInformado = Number(body.meses);
  if (mesesInformado >= 1 && mesesInformado <= 12) return Math.round(mesesInformado);
  const mesEscolhido = Number(body.mes);
  if (mesEscolhido >= 1 && mesEscolhido <= 12) return Math.max(1, 13 - mesEscolhido);
  return mesesAteFimDoAno();
}
...
async function salvarAjusteRemanejamentoComConn(conn, body, file, escopo) {
  ...
  // No ajuste, a quantidade de meses é informada diretamente (1..12). Sem valor
  // válido, cai no cálculo por mês (mês → dezembro) e, em último caso, no padrão.
  const meses = resolverMesesAjuste(body);
