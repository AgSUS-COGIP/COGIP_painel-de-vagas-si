...
const { limparValorDash, converterNumeroDash, normalizarChaveDash, extrairCompetenciaDash, obterUltimaAtualizacaoDash, somaServidor, formatDateInTimeZone, calcularOciosasServidor } = require("./utils");
...
const { dseiNoEscopo, obterChavesNomeEscopoComConn, erroEscopo } = require("./escopo");

// Filtra linhas (monitoramento ou remanejamento — ambas têm idDseiCasai) pelo
// escopo de DSEI do usuário. escopo.todos (ou ausente) => sem filtro.
function filtrarPorEscopo(rows, escopo) {
  if (!escopo || escopo.todos) return rows || [];
  return (rows || []).filter(r => dseiNoEscopo(escopo, r.idDseiCasai));
}

// Totais globais (SUM em toda a base) só valem para quem vê tudo. Para um usuário
// restrito, retorna null para que calcularIndicadoresServidor some a partir das
// linhas JÁ filtradas (contratados/indicadores do escopo, não globais).
async function totaisParaEscopo(escopo) {
  if (escopo && !escopo.todos) return null;
  return obterMonitoramentoTotaisComCache();
}

async function getDashboardData(escopo) {
  const rows = filtrarPorEscopo(await obterMonitoramentoRowsComCache(), escopo);
  const totaisMonitoramento = await totaisParaEscopo(escopo);
  const remanejamentos = filtrarPorEscopo(await obterRemanejamentoListaComCache(), escopo);
  const remanejamentoCadastro = filtrarPorEscopo(await obterRemanejamentoCadastroComCache(), escopo);
...
async function getDashboardResumoData(escopo) {
  const rows = filtrarPorEscopo(await obterMonitoramentoRowsComCache(), escopo);
  const totaisMonitoramento = await totaisParaEscopo(escopo);
...
async function getDashboardApoioData(escopo) {
  const rows = filtrarPorEscopo(await obterMonitoramentoRowsComCache(), escopo);
  const remanejamentos = filtrarPorEscopo(await obterRemanejamentoListaComCache(), escopo);
  const remanejamentoCadastro = filtrarPorEscopo(await obterRemanejamentoCadastroComCache(), escopo);
...
async function getVagasData(escopo) {
  const rows = filtrarPorEscopo(await obterMonitoramentoRowsComCache(), escopo);
  const totaisMonitoramento = await totaisParaEscopo(escopo);
...
async function getAlertasData(escopo) {
  const rows = filtrarPorEscopo(await obterMonitoramentoRowsComCache(), escopo);
  const observacoes = await getAlertasObservacoesMap(escopo);
...
async function getAlertasObservacoesMap(escopo) {
  let conn = null;
  try {
    ...
    // Escopo por DSEI: a observação guarda só o NOME do DSEI, então o filtro é por
    // nome normalizado (fail-closed — observação sem DSEI casado fica de fora p/ o restrito).
    const chavesNome = (escopo && !escopo.todos) ? await obterChavesNomeEscopoComConn(conn, escopo) : null;

    const mapa = {};
    (rows || []).forEach(row => {
      const chave = limparValorDash(row.CHAVE_ALERTA);
      if (!chave) return;
      if (chavesNome && !chavesNome.has(normalizarChaveDash(limparValorDash(row.DSEI_CASAI)))) return;
      mapa[chave] = {
...
async function salvarObservacaoAlertaComConn(conn, body, escopo) {
  ...
  // Escopo por DSEI: usuário restrito só anota alertas do(s) seu(s) DSEI(s).
  // Filtro por nome (fail-closed) — a observação guarda o nome do DSEI, não o id.
  if (escopo && !escopo.todos) {
    const chavesNome = await obterChavesNomeEscopoComConn(conn, escopo);
    if (!chavesNome.has(normalizarChaveDash(dsei))) {
      throw erroEscopo("Você não pode salvar observação de um alerta de DSEI fora do seu escopo.");
    }
  }
...
async function buscarMonitoramentoTotaisComConn(conn) {
  try {
    ...
    return {
      ...
      admissaoProgramada: converterNumeroDash(row.admissao_programada)
    };
  } catch (err) {
...
function mapMonitoramentoRow(row) {
  ...
  // Contratados com data de admissão futura (ainda não começaram). Já estão
  // embutidos em total_contratados_geral; entram na coluna "Total de Trabalhadores"
  // (com bandeirinha), mas são descontados do card "Trabalhadores Contratados".
  // 0 quando a coluna não está na view (flag desligada).
  const admissaoProgramada = converterNumeroDash(row.admissao_programada);
...
function calcularIndicadoresServidor(rows, totaisMonitoramento) {
  ...
  // Admissões programadas (total). Preferência pelo SUM da view (totais); senão soma as linhas.
  const admissaoProgramada = totaisMonitoramento && totaisMonitoramento.admissaoProgramada !== undefined
    ? Number(totaisMonitoramento.admissaoProgramada || 0)
    : somaServidor(rows, "admissaoProgramada");
  // Valor exclusivo do card "Trabalhadores Contratados": desconta as admissões
  // programadas. NÃO altera `contratados` (que segue alimentando vagas ociosas,
  // % preenchimento e % indígenas com o total cheio, como hoje).
