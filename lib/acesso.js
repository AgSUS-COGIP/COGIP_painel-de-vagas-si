...
async function aprovarSolicitacaoComConn(conn, idSolicitacao, adminEmail, opcoes = {}) {
  ...
  try {
    ...
    await conn.commit();
    return { id, email, status: "APROVADO" };
  } catch (err) {
    await conn.rollback();
    throw err;
  }
...
module.exports = { garantirTabelaSolicitacoesAcesso, salvarSolicitacaoAcessoComConn, obterListasAcesso, obterSituacaoAcessoComConn, listarSolicitacoesComConn, aprovarSolicitacaoComConn, recusarSolicitacaoComConn, excluirUsuarioComConn };
