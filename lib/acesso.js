...
const { aplicarEscopoSqlComConn, resolverIdUnidadePorNomeComConn } = require("./escopo");
...
// Deriva a unidade de escopo a partir da solicitação:
//   { restringir:false }                 -> Sede AgSUS (sem unidade) => vê todos os DSEIs
//   { restringir:true, nome:"DSEI ..." } -> aba DSEI/SESAI, CASAI, ou escritório mapeado
//   { restringir:true, nome:null }       -> escritório regional/desconhecido => fail-closed
function unidadeEscopoDaSolicitacao(sol) {
  const casai = limparValorDash(sol && sol.CASAI);
  if (casai) return { restringir: true, nome: casai };
  const dsei = limparValorDash(sol && sol.DSEI);
  if (!dsei) return { restringir: false, nome: null };
  if (/^ESCRITORIO/i.test(dsei)) {
    return { restringir: true, nome: _escritorioParaDseiNorm.get(normalizarChaveDash(dsei)) || null };
  }
  return { restringir: true, nome: dsei };
}
...
async function aprovarSolicitacaoComConn(conn, idSolicitacao, adminEmail, opcoes = {}) {
  ...
  try {
    ...
    //  - DSEI/SESAI, CASAI ou escritório  -> restrito ao id daquele DSEI/CASAI;
    //  - unidade não resolvida (regional/grafia divergente) -> restrito SEM DSEI
    //    (fail-closed: não vê nada até um super admin ajustar o escopo em Perfis).
    const uni = unidadeEscopoDaSolicitacao(sol);
    if (!uni.restringir) {
      await aplicarEscopoSqlComConn(conn, email, true, []);
    } else {
      const idDsei = uni.nome ? await resolverIdUnidadePorNomeComConn(conn, uni.nome) : null;
      if (!idDsei) {
        console.warn(
      ...
      await aplicarEscopoSqlComConn(conn, email, false, idDsei ? [idDsei] : []);
    }
...
module.exports = { garantirTabelaSolicitacoesAcesso, salvarSolicitacaoAcessoComConn, obterListasAcesso, obterSituacaoAcessoComConn, listarSolicitacoesComConn, aprovarSolicitacaoComConn, recusarSolicitacaoComConn, excluirUsuarioComConn, unidadeEscopoDaSolicitacao };
