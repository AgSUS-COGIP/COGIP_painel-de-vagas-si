...
const DASH_CONFIG = {
  ...
  // Liga o tratamento de "Admissão Programada" (contratados com data de admissão
  // futura) nos painéis Visão Geral e Vagas: desconta do card "Trabalhadores
  // Contratados" e marca a bandeirinha laranja na coluna "Total de Trabalhadores".
  // REQUER a coluna `admissao_programada` na MONITORAMENTO_VIEW (contagem de
  // contratados com data de admissão futura, por DSEI/cargo) — já criada no banco.
  // Vem LIGADO por padrão; é só um interruptor de segurança: se um dia a coluna
  // sumir da view (senão a consulta quebra com "Unknown column"), defina
  // MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA=false no .env para desativar.
  MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA:
    String(process.env.MONITORAMENTO_TEM_ADMISSAO_PROGRAMADA || "true").toLowerCase() !== "false",
...
// Configuração do segundo banco (PostgreSQL). É preguiçosa como a do MySQL: só
// é montada/validada quando o pool Postgres é realmente solicitado, então deixar
// as variáveis PG_* vazias no .env não impede a aplicação de subir.
function getPostgresConfig() {
  const url = String(process.env.PG_URL || "").trim();
  const user = String(process.env.PG_USER || "").trim();
  const password = String(process.env.PG_PASSWORD || "").trim();
  const host = String(process.env.PG_HOST || "").trim();

  // PG_SSL=true liga TLS (comum em Postgres na nuvem). rejectUnauthorized=false
  // aceita certificados autoassinados/gerenciados que não estão na cadeia local.
  const ssl = /^(1|true|yes|on)$/i.test(String(process.env.PG_SSL || "").trim())
    ? { rejectUnauthorized: false }
    : false;

  // Uma connection string (PG_URL) tem precedência: o driver pg extrai host,
  // porta, usuário, senha e banco dela sozinho.
  if (url) {
  ...
  if (!user || !password || !host) {
    throw new Error(
      "Configuração PostgreSQL ausente. Defina PG_HOST, PG_PORT, PG_DATABASE, " +
      "PG_USER e PG_PASSWORD (ou uma única PG_URL) no arquivo .env."
    );
  }
  ...
  return {
    ...
    user,
    password,
    database: String(process.env.PG_DATABASE || "").trim(),
    ssl
  };
}
...
module.exports = { DASH_CONFIG, getMysqlConfig, getPostgresConfig, resolverPortaAplicacao, parseJdbcUrl };
