...
| **escala-trabalho.js** | Aba de Escala de Trabalho: maquete autocontida (dados de exemplo, sem backend), no padrão de Gestão de Férias / Processos Seletivos. Filtros (DSEI/polo/UBSI/cargo/escala/situação + busca) filtram a tabela "Profissionais e Escalas"; alerta de profissionais sem escala (chips); detalhamento de plantonistas (grade dia/noite/folga) e de escalas de território (ida/retorno/dias); resumo (KPIs) e legenda. Escrita (Nova Escala/editar/excluir) é só demonstração e exige Editor (nível ≥ 2). Ligada no init via `configurarEscalaTrabalho()` e re-renderizada ao abrir via `renderEscalaTrabalhoAoMostrar()`. |
